    ...
    public class OracleDatabaseCreatorTest
    {
        ...
        private async Task Create_checks_for_existence_and_retries_until_it_passes(int errorNumber, bool async)
        {
            var customServices = new ServiceCollection()
                .AddScoped<IOracleConnection, FakeOracleConnection>()
            ...
            var contextServices = OracleTestHelpers.Instance.CreateContextServices(customServices);

            var connection = (FakeOracleConnection)contextServices.GetRequiredService<IOracleConnection>();

            connection.ErrorNumber = errorNumber;
            connection.FailureCount = 5;

            var creator = (OracleDatabaseCreator)contextServices.GetRequiredService<IRelationalDatabaseCreator>();
        ...
        private class FakeOracleConnection : OracleRelationalConnection
        {
            private readonly IDbContextOptions _options;

            public FakeOracleConnection(IDbContextOptions options, RelationalConnectionDependencies dependencies)
                : base(dependencies)
            {
                _options = options;
            }
            ...
            public override bool Open(bool errorsExpected = false)
            {
                if (++OpenCount < FailureCount)
                {
                    ...
                    throw OracleExceptionFactory.CreateOracleException(ErrorNumber);
                }
