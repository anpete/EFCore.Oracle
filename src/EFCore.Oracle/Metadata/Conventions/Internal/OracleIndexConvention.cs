...
{
    public class OracleIndexConvention :
        ...
        IPropertyAnnotationChangedConvention
    {
        ...
        public OracleIndexConvention([NotNull] ISqlGenerationHelper sqlGenerationHelper)
        {
        ...
        {
            if (name == OracleAnnotationNames.Clustered)
            {
                SetIndexFilter(indexBuilder);
            }
        ...
        private InternalIndexBuilder SetIndexFilter(InternalIndexBuilder indexBuilder, bool columnNameChanged = false)
        {
            ...
            if (index.IsUnique
                && indexBuilder.Metadata.Oracle().IsClustered != true
                && index.Properties
                    .Any(property => property.IsColumnNullable()))
            {
                if (columnNameChanged
                    || index.Oracle().Filter == null)
                {
                    indexBuilder.Oracle(ConfigurationSource.Convention).HasFilter(CreateIndexFilter(index));
                }
            }
            else
            {
                if (index.Oracle().Filter != null)
                {
                    indexBuilder.Oracle(ConfigurationSource.Convention).HasFilter(null);
                }
            }
        ...
        private string CreateIndexFilter(IIndex index)
        {
            var nullableColumns = index.Properties
                ...
                .Select(property => property.Oracle().ColumnName)
                .ToList();
