...
{
    public class OracleModelValidatorTest : RelationalModelValidatorTest
    {
        ...
        public virtual void Passes_for_incompatible_indexes_within_hierarchy_when_one_name_configured_explicitly_for_sqlServer()
        {
            ...
            Assert.Equal("IX_Animal_Name", index1.Oracle().Name);
            Assert.Equal("IX_Animal_Name0", index2.Relational().Name);
            Assert.Equal("IX_Animal_Name0", index2.Oracle().Name);
        }
        ...
        {
            var modelBuilder = new ModelBuilder(TestRelationalConventionSetBuilder.Build());
            modelBuilder.Entity<Dog>().Property<byte>("Bite").UseOracleIdentityColumn();

            VerifyWarning(OracleStrings.LogByteIdentityColumn.GenerateMessage("Bite", nameof(Dog)), modelBuilder.Model);
        }
