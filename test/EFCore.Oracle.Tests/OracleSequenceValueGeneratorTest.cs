...
{
    public class OracleSequenceValueGeneratorTest
    {
        [Theory]
        [InlineData(false)]
        [InlineData(true)]
        ...
        private static IOracleConnection CreateConnection(IServiceProvider serviceProvider = null)
        {
            serviceProvider = serviceProvider ?? OracleTestHelpers.Instance.CreateServiceProvider();

            return OracleTestHelpers.Instance.CreateContextServices(serviceProvider).GetRequiredService<IOracleConnection>();
        }
